...
def detect_load_model(model_source, reference, epoch, device):
    ...
    if epoch == 'last' or epoch == 'best':
        model.load_state_dict(torch.load(os.path.join(model_source, f'{epoch}_epoch.pt')))
    else:
        model.load_state_dict(torch.load(os.path.join(model_source, 'best_epoch.pt')))
    model.to(device)
    return model
...
def eval_compute_gradcam(shared_obj, image_obj):
    ...
    # Process image
    #rgb_image = np.array(Image.fromarray(np.uint8(
    #    plt.get_cmap('gray')(image.numpy().squeeze()) * 255)).convert('RGB')) / 255
    #input_tensor = image.unsqueeze(0).to(device, dtype=torch.float)
